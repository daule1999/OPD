...
class PatientCrud {
  ...
  createTable() {
    ...
    const sql = `CREATE TABLE IF NOT EXISTS Patient(Pid INTEGER primary key,Uid INTEGER ,Tid INTEGER,name text,address text,age INTEGER,gender text,phone text,currentTemp INTEGER,currentBp text,currentOxygen text,dateOfAppoint text,dateOfBooking text,doctor INTEGER DEFAULT 1,sqltime TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL)`
  ...
  insertData(patient) {
    console.log("insert.....Data ...", patient, patient.phone)
    return this.dao.run("INSERT INTO Patient (Uid,Tid,name,address,age,gender,doctor,phone,currentTemp,currentBp,currentOxygen,dateOfAppoint,dateOfBooking) values(?,?,?,?,?,?,?,?,?,?,?,?,?)",
      [patient.Uid, patient.Tid, patient.name, patient.address, patient.age, patient.gender, patient.doctor, patient.phone, patient.currentTemp, patient.currentBp, patient.currentOxygen, patient.dateOfAppoint, patient.dateOfBooking])
  }
  getAllPatients() {
